...
@section('admin-tittle')Format Surat @endsection
...
<div class="page-header">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
         <a class="btn btn-primary float-right my-3" href="javascript:void(0)" id="createNew"> Create Format Surat</a>
        </div>
     </div>
</div>
                    ...
                    <div class="modal-body">
                        <form id="ajaxForm" name="ajaxForm" class="form-horizontal">
                            ...
                            <div class="form-group">
                                <label for="kode_format">Kode Surat</label>
                                <input type="text" name="kode_format" id="kode_format" class="form-control" >
                            </div>
                            <div class="form-group">
                                <label for="nama_format">Nama Surat</label>
                                <input type="text" name="nama_format" id="nama_format" class="form-control" >
                            </div>
                            <div class="form-group">
                                <label for="file_format">Upload File</label>
                                <input type="file" accept=".doc, .docx" name="file_format" id="file_format" class="form-control" >
                            </div>
...
<script>
    $(function () {
        ...
        $('#createNew').click(function () {
                ...
                $('#titleModal').html("Create Format Surat");
                $('#ajaxModal').modal('show');
        });

        $('body').on('click', '.editData', function () {
                var id = $(this).data('id');
                $.get('formatsurat/' + id + '/edit', function (data) {
                    $('#titleModal').html("Edit " + data.nama_format);
                    $('#saveBtn').val("editDataa");
                    $('#ajaxModal').modal('show');
                    $('#id').val(data.id);
                    $('#nama_format').val(data.nama_format);
                    $('#kode_format').val(data.kode_format);
        ...
        $('#ajaxForm').submit(function (e) {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    toast({
                        type: 'success',
                        title: data.nama_format + ' data is successfully saved',
                        padding: '2em',
                    })
                },
                error: function (data) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        setTimeout(function () {
                            ...
                            toast({
                                type: 'success',
                                title: ' Format Surat is successfully delete',
                                padding: '2em',
                            })
                        });
